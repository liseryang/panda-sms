...
<#--
 * messageText as mt
 *在资源文件中获得key为code的value 如果没有找到怎返回text指定的值
 * Macro to translate a message code into a message,
...
<#--
 * messageArgs as ma
 *在资源文件中获得key为code的value，args为传入value的差数
 ...
 *在资源文件中获得key为code的value，args为传入value的差数，如果没有找到怎返回text指定的值
 * Macro to translate a message code with arguments into a message,
 * using the given default text if no message found.
 -->
...
<#--
 * themeText
 *在资源文件中得到一个指定code的消息，如果没有找到就返回一个text为默认值
 * Macro to translate a theme message code into a message,
 * using the given default text if no message found.
 -->
...
<#--
 * themeArgsText
 *在资源文件中得到一个指定code的消息，如果没有找到就返回一个text为默认值并且将参数设置进去
 * Macro to translate a theme message code with arguments into a message,
 * using the given default text if no message found.
 -->
...
<#macro url relativeUrl>
	${springMacroRequestContext.getContextPath()}${relativeUrl}
</#macro>
